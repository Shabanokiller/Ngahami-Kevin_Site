    ...
    isset($pathologie) && !empty($pathologie))
        {
            ...
            $sql = "UPDATE `visite` SET  `pathologie` = :pathologie WHERE `visite`.`Nom Du Lieu` = :lieu AND `visite`.`Numéro Civique` = :numero AND `visite`.`Rue` = :rue AND `visite`.`Ville` = :ville AND `visite`.`Province` = :province AND `visite`.`mail` = :mail";
